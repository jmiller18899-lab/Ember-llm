...
def load_config(path: Path) -> dict:
    ...
    for name, low, high in (("history_length", 2, 64), ("bits", 2, 32), ("hidden_size", 4, 128),
...
def fit_one(cfg, mode, partitions, normalization, output_dir):
    ...
    for step in range(1, cfg["max_steps"] + 1):
        ...
        prediction, auxiliary = model(train_x[index])
        ...
        if step % cfg["eval_every"] == 0 or step == cfg["max_steps"]:
            model.eval()
            with torch.no_grad():
                validation_loss = float(forecast_loss(model(val_x)[0], val_y))
    ...
    model.eval()
    with torch.no_grad():
        validation_prediction = model(val_x)[0] * y_scale + y_mean
        test_prediction = model(test_x)[0] * y_scale + y_mean
    checkpoint = {"schema_version": cfg["schema_version"], "model_args": model_args,
                  ...
                  "config": cfg, "selected_step": best_step}
...
def predict_checkpoint(path: Path, features, feature_names: list[str]) -> dict:
    ...
    model.eval()
    with torch.no_grad():
        raw = model(((x - norm["feature_mean"]) / norm["feature_scale"]).unsqueeze(0))[0][0]
...
def run(cfg: dict, data: Path, output_dir: Path, data_kind: str = "user_supplied_unverified") -> dict:
    torch.set_num_threads(1)
    rows = load_examples(data, cfg)
    parts, split = chronological_split(rows)
    ...
    models = {mode: fit_one(cfg, mode, prepared, normalization, output_dir) for mode in MODES}
    ...
    report = {"status": "completed", "data_kind": data_kind, "schema_version": cfg["schema_version"],
              ...
              "split": split, "constant_baseline": baseline, "models": models,
