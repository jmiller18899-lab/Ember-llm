"""Arithmetic tool v3 (candidate). Changes from v2 (jobs/ember_arith_tool.py, unchanged): label numbers such as years and
room/gate/bus numbers are masked before solving; "leave/quit/cancel" count as removals; "goes into … at" is a start and
"needs/requires N minutes" a duration.
...
SUB_CUE=re.compile(r"\b(remove[sd]?|removing|take[sn]?|took|taken|sold|sell[s]?|use[sd]?|lose[s]?|lost|gave|give[sn]?|broke|broken|spent|spend[s]?|ate|eaten|eat[s]?|drop(?:ped|s)?|discard(?:ed|s)?|minus|subtract(?:ed|s)?|shipped out|went out|borrowed|drained|poured out|carried out|taken out|leaked|leave|leaves|left(?! over)|quit|quits|dropped out|drop out|cancel(?:s|led|ed)?)\b",re.I)
...
START_VERB=re.compile(r"\b(leaves?|left|departs?|departed|starts?|started|begins?|began|opens?|kicks? off|sets? off|set out|takes? off|boards?|sails?|sailed|put|placed|goes into|go into|went into|goes in|went in)\b",re.I)
DUR=re.compile(r"\b(?:(\d+)\s*(?:hours?|hrs?|h)\b(?:\s*(?:and\s*)?(\d+)\s*(?:minutes?|mins?|m)\b)?|(\d+)\s*(?:minutes?|mins?)\b)",re.I)
DUR_CUE=re.compile(r"\b(takes?|took|lasts?|lasted|runs?|ran|for|is|was|needs?|needed|requires?|required|travel time|duration|ride|trip|long|later|after)\b",re.I)
...
# Numbers that name something rather than count it: years and identifiers. Only the digits are blanked, so offsets and
# wording stay intact and the solvers still see every real quantity.
LABEL=re.compile(r"\b(?:room|rooms|suite|apartment|apt|unit|gate|bus|route|platform|track|aisle|locker|case|record|request|ticket|invoice|floor|building|no\.|number)\s*#?\s*\d+\b|#\s*\d+\b|\b(?:in|since|from|founded in|established in|built in|opened in|year)\s+(?:1[89]|20)\d\d\b",re.I)
def mask_labels(p): return LABEL.sub(lambda m:re.sub(r"\d"," ",m.group()),p)
def solve(prompt):
    """(kind, answer) for the single solver that fires, else None. Two solvers firing is treated as ambiguous."""
    prompt=mask_labels(prompt)
