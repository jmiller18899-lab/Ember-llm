...
def load_rows():
 ...
 if p.exists(): src=p.read_text()
 else:
  # Standalone run (e.g. `uv run <raw url>`): fetch the suite builder next to this script on GitHub.
  import urllib.request
  src=urllib.request.urlopen(SUITE_URL,timeout=60).read().decode()
 m={"__name__":"suite"}; exec(compile(src,"build_ember_promotion_suite_v2.py","exec"),m)
