...
from ember_qwen_sft_v3 import REPO
...
def main():
    ...
    guard = "launches/sft-v3.json"
    if api.file_exists(REPO, guard) or api.file_exists(REPO, "adapter_model.safetensors"):
        raise RuntimeError("Experiment already reserved; refusing duplicate GPU billing")
    source = Path("jobs/ember_qwen_sft_v3.py").read_bytes()
    receipt = {"status":"reserved", "source_commit":os.environ["GITHUB_SHA"],
        "source_sha256":hashlib.sha256(source).hexdigest(),
        "github_run_id":os.environ["GITHUB_RUN_ID"], "hardware":"l4x1",
        "timeout_seconds":5400, "listed_hourly_usd":0.80, "maximum_compute_usd":1.20}
    ...
    try:
        job = api.run_uv_job("jobs/ember_qwen_sft_v3.py", python="3.11", flavor="l4x1", timeout=5400,
            name="ember-qwen35-2b-sft-v3", secrets={"HF_TOKEN":os.environ["HF_TOKEN"]},
