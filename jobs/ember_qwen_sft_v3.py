...
# v3: fixed continuation with replay and separately authored confirmation prompts.
PARENT = "Jmiller18899/ember-qwen3.5-2b-sft-v2"
PARENT_REVISION = "622117811fc97436e23226418647d29efa85714e"
OLD_DATA = data
...
def data():
    ...
    assert len(sets["train"]) == 864 and len(sets["dev"]) == 112
    return sets
...
def main():
    ...
    manifest = {"model": MODEL, "revision": REVISION, "system": SYSTEM,
        ...
        "parent": PARENT, "parent_revision": PARENT_REVISION, "max_steps": 108, "learning_rate": 0.000005, "production_ready": False,
    ...
    if not args.preflight:
        model = PeftModel.from_pretrained(model, PARENT, revision=PARENT_REVISION, is_trainable=True)
    model.config.use_cache = False
    lora = LoraConfig(r=8, lora_alpha=16, lora_dropout=0.05, bias="none", task_type="CAUSAL_LM",
        target_modules=["q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj"])
    config = SFTConfig(output_dir=str(out), max_steps=2 if args.preflight else 108,
    ...
    trainer = SFTTrainer(model=model, args=config, train_dataset=encoded["train"],
        eval_dataset=encoded["dev"], processing_class=tokenizer, data_collator=collator, peft_config=lora if args.preflight else None)
    ...
    if args.preflight:
        ...
        return
    import trackio
    trackio.init(project="ember-qwen-sft", name="qwen35-2b-v3-grounding", space_id="Jmiller18899/trackio", config=manifest)
    ...
    def evaluate(label):
        ...
        return results
    baseline = evaluate("parent_v2")
    with trainer.model.disable_adapter():
        original_base = evaluate("original_base")
    trainer.model.train()
    model.config.use_cache = False
    trainer.train()
    trainer.save_model()
    ...
    summary = {"baseline":before, "final":after,
        ...
        "original_base":counts(original_base),
        ...
        "training_examples":864, "steps_completed":trainer.state.global_step,
