"""Run Ember v0.0.50 frozen-teacher KL minimal-delta CPU canary."""
...
def summary(report: dict) -> str:
    ...
    return "\n".join([
        f"# Ember v0.0.50 teacher-KL CPU canary: {report['status']}", "",
        "Restarted from untouched v0.0.31 step 479. Candidate step was selected only from fresh synthetic development/distillation evidence.", "",
...
def main() -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("v050-results"))
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="ember-v050-") as td:
            ...
            if source.get("step") != 479 or source.get("train_config", {}).get("version") != cfg["source_version"]:
                raise ValueError("v0.0.50 source is not untouched v0.0.31 step 479")
            ...
            entry_train = v048d.build_cases({k: values["train"][k] for k in sorted(data.ENTRY_SUBTYPES)}, "v050_entry_train")
            place_train = v048d.build_cases({k: values["train"][k] for k in sorted(data.PLACEMENT_SUBTYPES)}, "v050_place_train")
            entry_dev = v048d.build_cases({k: values["development"][k] for k in sorted(data.ENTRY_SUBTYPES)}, "v050_entry_dev")
            place_dev = v048d.build_cases({k: values["development"][k] for k in sorted(data.PLACEMENT_SUBTYPES)}, "v050_place_dev")
            ...
            for step in range(1, int(cfg["max_optimizer_steps"]) + 1):
                ...
                if not bool(torch.isfinite(total).item()):
                    raise ValueError("non-finite v0.0.50 canary loss")
                total.backward()
                ...
                if step == 1 or step % int(cfg["checkpoint_interval"]) == 0:
                    ...
                    print(json.dumps({"event": "teacher_kl_step", **event}), flush=True)
            ...
            after_entry = latest_entry
            after_place = latest_place
            after_tool = latest_tool
            after_copy_distill = latest_copy_distill
            ...
            report.update(
                ...
                before={
                    ...
                    "tool_distill": before_tool, "copy_distill": before_copy_distill,
                ...
                decision=decision,
                meaning="Frozen-teacher minimal-delta CPU canary only; no GPU or promotion authorization.",
            )
