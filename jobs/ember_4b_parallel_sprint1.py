...
def main():
    ...
    def evaluate():
        ...
        return out
    before=evaluate()
    ta=TrainingArguments(output_dir=f"sprint-{lane}",max_steps=20,per_device_train_batch_size=1,gradient_accumulation_steps=4,learning_rate=1.5e-6,warmup_steps=2,bf16=True,gradient_checkpointing=True,gradient_checkpointing_kwargs={"use_reentrant":False},save_strategy="no",report_to=[],seed=431,data_seed=431)
    tr=Trainer(model=model,args=ta,train_dataset=ds,data_collator=coll); tr.train()
    outdir=Path(f"sprint-{lane}-candidate"); tr.save_model(str(outdir)); tok.save_pretrained(str(outdir))
    ...
    Path(f"sprint-{lane}-summary.json").write_text(json.dumps(summary,indent=2))
    repo=f"Jmiller18899/ember-qwen3.5-4b-sprint1-{lane}"; api.create_repo(repo,private=True,exist_ok=True)
    ...
    api.upload_file(repo_id=repo,path_in_repo="summary.json",path_or_fileobj=f"sprint-{lane}-summary.json",commit_message="Save sprint lane gate evidence")
    ...
    print("SPRINT_SUMMARY "+json.dumps(summary),flush=True)
