"""Deterministic, held-out-safe instruction data for Ember v0.0.9.

The official promotion prompts are never used here.  This module creates a
small balanced curriculum that teaches three decisions:

* call the requested tool using Ember's canonical JSON envelope;
* answer directly when no tool is needed; and
* explain supplied tool results without calling a tool again.
"""
...
TRAIN_VALUES = {
    "locations": [
        ...
        "Seattle", "Austin", "Miami", "Denver", "Phoenix", "Portland",
    ],
    "timezones": [
        ("London", "Europe/London"), ("Sydney", "Australia/Sydney"),
        ("Berlin", "Europe/Berlin"), ("Honolulu", "Pacific/Honolulu"),
        ("Seoul", "Asia/Seoul"), ("Anchorage", "America/Anchorage"),
        ("Dublin", "Europe/Dublin"), ("Singapore", "Asia/Singapore"),
    ],
    "services": ["api", "scheduler", "database", "worker", "dashboard", "queue"],
}

VALIDATION_VALUES = {
    "locations": ["Toledo", "Cleveland", "Milwaukee", "Nashville", "Raleigh", "Boise"],
    "timezones": [
        ("Paris", "Europe/Paris"), ("Auckland", "Pacific/Auckland"),
        ("Mumbai", "Asia/Kolkata"), ("Vancouver", "America/Vancouver"),
        ("Rome", "Europe/Rome"), ("Reykjavik", "Atlantic/Reykjavik"),
    ],
    "services": ["search", "notifier", "billing", "cache", "ingest", "auth"],
}
...
SEARCH_TOPICS = (
    ...
    "this week's Linux kernel news", "the latest SQLite release notes",
    "current browser compatibility data", "recent renewable energy headlines",
    "the newest TypeScript release", "today's space weather bulletin",
    "the current Kubernetes release", "recent robotics research news",
)
...
DIRECT_TOPICS = (
    "cache", "checksum", "retry", "checkpoint", "queue", "tokenizer",
...
def _system(kind: str, variant: int, tool: str | None = None) -> str:
    if kind == "tool_call":
        choices = (
            f"You are Ember. Use the {tool} tool for this request and provide JSON arguments.",
            f"You are Ember. Current information requiring {tool} must use that exact tool name.",
            f"You are Ember. Call {tool} with a compact JSON argument object.",
        )
        return choices[variant % len(choices)]
    if kind == "direct_response":
        choices = (
            "You are Ember. Answer directly when no external tool is needed.",
        ...
        return choices[variant % len(choices)]
    choices = (
        "You are Ember. Explain supplied tool results without calling the tool again.",
        "You are Ember. A tool result is already available; answer the user directly from it.",
        "You are Ember. Summarize the supplied result and do not make another tool call.",
    )
    return choices[variant % len(choices)]
...
def _tool_call_example(split: str, index: int, values: dict) -> dict:
    ...
    if tool == "weather":
        ...
        split_offset = 503 if split == "validation" else 0
        left = 18 + (((variant + split_offset) * 17) % 470)
        right = 3 + (((variant + split_offset) * 11) % 89)
        operators = ("+", "-", "*", "/")
        operator = operators[variant % len(operators)]
        user = (
            f"Use a calculator to evaluate {left} {operator} {right}."
            if variant % 2 == 0
            else f"Please calculate the expression {left}{operator}{right}."
        )
        arguments = {"expression": f"{left}{operator}{right}"}
...
def _result_example(split: str, index: int, values: dict) -> dict:
    ...
    if subtype == 0:
        ...
        split_offset = 307 if split == "validation" else 0
        left = 20 + (((variant + split_offset) * 13) % 170)
        right = 2 + (((variant + split_offset) * 5) % 31)
        answer = left * right
...
def build_sft_examples(split: str, total: int, seed: int = 20260828) -> list[dict]:
...
def validate_examples(examples: Iterable[dict]) -> None:
    ...
    for row in rows:
        ...
        if not str(row.get("completion", "")).endswith("<|endoftext|>\n"):
            raise ValueError(f"SFT completion must end with endoftext: {row.get('id')}")
        if row["kind"] == "tool_call":
            marker, payload_text = row["completion"].split("\n", 1)
            ...
            if not isinstance(payload.get("arguments"), dict) or not payload["arguments"]:
                raise ValueError(f"tool arguments must be a non-empty object: {row.get('id')}")
        elif "<|tool|>" in row["completion"]:
