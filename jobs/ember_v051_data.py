"""Fresh disjoint data builder for Ember v0.0.51 placement-focused canary."""
...
from jobs import ember_v049_replay_compat  # noqa: F401; patches widened v0.0.49 lineage
from jobs import ember_v050_data as v050

base = v050.base
copy_data = v050.copy_data
v044 = v050.v044
v048d = v050.v048d
DEFAULT_CONFIG = Path(__file__).resolve().parents[1] / "config/ember_placement_teacher_kl_v0.0.51.json"
ENTRY_SUBTYPES = set(v050.ENTRY_SUBTYPES)
PLACEMENT_SUBTYPES = set(v050.PLACEMENT_SUBTYPES)
TARGET_SUBTYPES = ENTRY_SUBTYPES | PLACEMENT_SUBTYPES


def load_config(path: Path = DEFAULT_CONFIG) -> dict:
    cfg = json.loads(path.read_text(encoding="utf-8"))
    if path.resolve() != DEFAULT_CONFIG.resolve() or cfg.get("version") != "0.0.51":
        raise ValueError("unsupported v0.0.51 configuration")
    if cfg.get("cpu_learning_authorized") is not True:
        raise ValueError("v0.0.51 CPU learning authorization missing")
    ...
    if set(cfg.get("entry_subtypes", [])) != ENTRY_SUBTYPES or set(cfg.get("placement_subtypes", [])) != PLACEMENT_SUBTYPES:
        raise ValueError("v0.0.51 target subtype sets changed")
    ...
    if any(w <= 0 for w in weights) or abs(sum(weights) - 1.0) > 1e-9:
        raise ValueError("v0.0.51 weights must be positive and sum to one")
    if float(cfg["tool_kl_loss_weight"]) + float(cfg["copy_kl_loss_weight"]) < 0.75:
        raise ValueError("v0.0.51 requires at least 75% teacher preservation weight")
    if float(cfg["placement_loss_weight"]) <= float(cfg["entry_loss_weight"]):
        raise ValueError("v0.0.51 must be placement-focused")
    if not 0 < float(cfg["learning_rate"]) <= 1.5e-7:
        raise ValueError("v0.0.51 learning rate exceeds placement-focused bound")
    if not 1 <= int(cfg["max_optimizer_steps"]) <= 60:
        raise ValueError("v0.0.51 max steps exceed bound")
    if int(cfg["checkpoint_interval"]) <= 0 or int(cfg["max_optimizer_steps"]) % int(cfg["checkpoint_interval"]) != 0:
        raise ValueError("invalid v0.0.51 checkpoint interval")
...
def historical_used_values() -> set[str]:
    used = set(v050.historical_used_values())
    old_cfg = v050.load_config()
    old_targets = v050.target_values(old_cfg)
    used.update(v for phase in old_targets.values() for values in phase.values() for v in values)
    for family in ("tool", "copy"):
        used.update(row["target"] for row in v050.distill_value_rows(old_cfg, family))
    return used
...
def target_values(cfg: dict) -> dict[str, dict[str, list[str]]]:
    ...
    for phase, count in counts.items():
        for subtype in sorted(TARGET_SUBTYPES):
            ...
            while len(values) < count:
                seed = copy_data._digest("v051-target", phase, subtype, i)
    ...
    if len(flat) != len(set(flat)) or set(flat) & set(copy_data.HELD_OUT_VALUES):
        raise ValueError("v0.0.51 target values overlap protected values")
    return out
...
def distill_value_rows(cfg: dict, family: str) -> list[dict]:
    ...
    for kind in copy_data.KINDS:
        for variant in range(int(copy_data.VARIANTS[kind])):
            ...
            i = 0
            while made < count:
                seed = copy_data._digest("v051-distill", family, kind, variant, i)
                value = copy_data._render(kind, variant, seed)
                i += 1
                if value in used:
                    continue
    ...
    if values & set(copy_data.HELD_OUT_VALUES):
        raise ValueError("v0.0.51 distillation values overlap familiar held-out battery")
    return rows
...
def prepare_tool_rows(teacher, tokenizer, torch, cfg: dict) -> tuple[list[dict], dict]:
    ...
    for index, row in enumerate(distill_value_rows(cfg, "tool")):
        case = {"id": f"v051_tool_distill_{row['kind']}_{index:03d}", **tool_prompt(row["kind"], row["target"])}
...
def prepare_copy_rows(teacher, tokenizer, torch, cfg: dict) -> tuple[list[dict], dict]:
    ...
    for index, row in enumerate(distill_value_rows(cfg, "copy")):
        ...
        if ids and not generation["completion"].lstrip().startswith(v048d.v045.TOOL):
            kept.append({"id": f"v051_copy_distill_{row['kind']}_{index:03d}", "kind": row["kind"], "prompt": prompt, "source_ids": ids})
