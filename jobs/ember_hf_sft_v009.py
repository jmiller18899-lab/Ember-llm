...
# ]
# ///
"""Run Ember v0.0.9's focused tool-routing supervised fine-tune.

This phase starts from the exact v0.0.8 checkpoint that passed the technical
evaluation but failed the absolute behavior gates.  It uses completion-only
loss over a deterministic curriculum that excludes the official promotion
prompts, persists resumable checkpoints, exports INT4, and leaves promotion to
a separate CPU evaluation.
"""
...
CONFIG_URL = "https://raw.githubusercontent.com/jmiller18899-lab/Ember-llm/main/config/ember_agent_tool_sft_v0.0.9.json"
CONFIG_SHA256 = "fccf9a1056868b260f55fee94eec3387bd4264b0539b4e305fe01260764781f2"
DATA_URL = "https://raw.githubusercontent.com/jmiller18899-lab/Ember-llm/main/jobs/ember_sft_data_v009.py"
DATA_SHA256 = "5c419ffb3ce84b5d31937914ee33ae0235638986dff2a2dccdfaa5842df0a5be"
...
def assert_no_live_duplicate(state: dict | None) -> None:
    ...
    if status in {"training_complete_pending_eval", "evaluation_complete", "complete"}:
        raise RuntimeError("v0.0.9 SFT is already complete; refusing a duplicate paid run")
    if status != "running":
        return
    updated_at = str(state.get("updated_at", ""))
    if not updated_at:
        raise RuntimeError("existing v0.0.9 run lock has no timestamp; manual review required")
    age = (utc_now() - parse_timestamp(updated_at)).total_seconds()
    if age < RUN_STATE_STALE_SECONDS:
        raise RuntimeError(f"another v0.0.9 SFT run appears active (state age {age:.0f}s)")
...
def load_data_module(path: Path):
    spec = importlib.util.spec_from_file_location("ember_sft_data_v009", path)
    ...
    for row in selected:
        ...
        if row["kind"] == "tool_call":
            ...
            passed = (
                ...
                and isinstance(payload.get("arguments"), (dict, str))
                and bool(payload.get("arguments"))
            )
        else:
            passed = "<|tool|>" not in completion and len(visible_text(completion)) >= 3
        results.append({
            "id": row["id"],
            "kind": row["kind"],
            "passed": bool(passed),
...
def resolve_resume(
    ...
    *,
    ...
    if present and present != required:
        raise RuntimeError("partial v0.0.9 resume state found; manual review required")
    if not present:
    ...
    for key, value in expected.items():
        if manifest.get(key) != value:
            raise RuntimeError(f"resume manifest {key} does not match this v0.0.9 run")
...
def upload_resume(
    ...
    *,
    ...
    api.upload_file(
        ...
        commit_message=f"Persist Ember v0.0.9 SFT resume checkpoint at step {step}",
    )
    api.upload_file(
        repo_id=repo_id,
        repo_type="model",
        ...
        commit_message=f"Persist Ember v0.0.9 SFT best checkpoint at step {step}",
    )
    if metrics_path.is_file():
        api.upload_file(
            ...
            commit_message=f"Persist Ember v0.0.9 SFT metrics at step {step}",
        )
    manifest = {
        "schema_version": 1,
        "status": "running",
    ...
    api.upload_file(
        ...
        commit_message=f"Update Ember v0.0.9 SFT resume manifest at step {step}",
    )
    return manifest


def main() -> None:
    parser = argparse.ArgumentParser(description="Run or preflight Ember v0.0.9 tool-routing SFT")
    ...
    if not args.preflight_only and not torch.cuda.is_available():
        raise RuntimeError("Ember v0.0.9 SFT requires the explicitly approved T4 GPU")

    api = HfApi(token=token)
    owner = api.whoami()["name"]
    source_repo = f"{owner}/ember-v0.0.8-t4"
    target_repo = f"{owner}/ember-v0.0.9-t4"
    trackio_space = f"{owner}/ember-trackio"

    with tempfile.TemporaryDirectory(prefix="ember-v009-sft-") as temporary:
        work = Path(temporary)
        ...
        if config.get("version") != "0.0.9" or config.get("phase") != "tool-routing-sft":
            raise RuntimeError("unexpected Ember v0.0.9 SFT configuration")
        if config.get("source_model_name") != "ember-v0.0.8-t4":
            raise RuntimeError("v0.0.9 SFT must initialize from the evaluated v0.0.8 checkpoint")
        if not bool(config.get("completion_only_loss")):
            raise RuntimeError("v0.0.9 SFT requires completion-only loss")
        ...
        if source_evaluation.get("label") != config["source_evaluation_label"]:
            raise RuntimeError("source checkpoint was not evaluated under the expected v0.0.8 label")
        if source_evaluation.get("evaluation_spec_sha256") != EVAL_SPEC_SHA256:
            raise RuntimeError("source evaluation used a different promotion specification")
        if not bool(source_evaluation.get("technical_pass")):
            raise RuntimeError("source v0.0.8 checkpoint did not pass the technical gate")
        if bool(source_evaluation.get("promotion", {}).get("promotion_eligible")):
            raise RuntimeError("source v0.0.8 is already promotion eligible; targeted repair is unnecessary")
        ...
        if args.preflight_only:
            ...
            print("EMBER_HF_V009_SFT_PREFLIGHT=PASS", flush=True)
            return
        ...
        trackio.init(
            project="ember",
            name="ember-v0.0.9-tool-routing-sft",
            embed=False,
            config={
                "version": "0.0.9",
                "phase": "tool-routing-sft",
                "hardware": "t4-small",
        ...
        state = upload_state(
            ...
            "Start Ember v0.0.9 tool-routing SFT",
        )
        ...
        try:
            for step in range(start_step, max_steps):
                ...
                if (
                    ...
                    state = upload_state(
                        ...
                        target_repo,
                        work,
                        {**state, **manifest, "phase": "tool-routing-sft"},
                        f"Update Ember v0.0.9 SFT state at step {step}",
                    )
            ...
            upload_state(
                ...
                "Record interrupted Ember v0.0.9 SFT",
            )
            raise
        finally:
            trackio.finish()
        ...
        if not int4_path.is_file() or int4_path.stat().st_size == 0:
            raise RuntimeError("v0.0.9 INT4 export did not produce a durable checkpoint")
        summary = {
            "schema_version": 1,
            ...
            "next_gate": "run candidate-v0.0.9 with the unchanged CPU promotion evaluation",
        }
        ...
        api.upload_folder(
            ...
            commit_message="Persist completed Ember v0.0.9 SFT checkpoints",
        )
        for local_path, remote_path, message in (
            (config_path, "config/ember_agent_tool_sft_v0.0.9.json", "Persist Ember v0.0.9 SFT config"),
            (data_path, "data/ember_sft_data_v009.py", "Persist Ember v0.0.9 data generator"),
            (train_data_path, "data/train.jsonl", "Persist Ember v0.0.9 SFT training data"),
            (validation_data_path, "data/validation.jsonl", "Persist Ember v0.0.9 SFT validation data"),
            (dataset_manifest_path, "data/manifest.json", "Persist Ember v0.0.9 SFT data manifest"),
            (summary_path, "training/summary.json", "Persist Ember v0.0.9 SFT summary"),
        ):
            api.upload_file(
                repo_id=target_repo,
                repo_type="model",
        ...
        if metrics_path.is_file():
            api.upload_file(
                ...
                commit_message="Persist Ember v0.0.9 SFT metrics",
            )
        if trackio_dir.exists():
            api.upload_folder(
                ...
                commit_message="Persist Ember v0.0.9 Trackio metrics",
            )
        ...
        if remote_best not in remote_files or remote_int4 not in remote_files:
            raise RuntimeError("completed v0.0.9 checkpoints were not persisted to the Hub")
        ...
        upload_state(
            ...
            "Complete Ember v0.0.9 tool-routing SFT",
        )
        ...
        print("EMBER_HF_V009_SFT=PASS", flush=True)
        ...
        print("NEXT_GATE=run eval-v009 on CPU", flush=True)
