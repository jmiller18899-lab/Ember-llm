...
# ]
# ///
"""Run Ember v0.0.13's short unique-copy canary from the accepted v0.0.9 checkpoint.

Do not initialize from the failed v0.0.11 or v0.0.12 runs. Those copied tool
JSON shape but not the requested values. This job uses EOS-aware greedy
decoding for its internal smoke gate and refuses promotion work when that gate
fails.
"""
...
ASSET_PIN = "42acfa8fcb36742955b90c2424f3eed2a3adf6d9"
CONFIG_URL = f"https://raw.githubusercontent.com/jmiller18899-lab/Ember-llm/{ASSET_PIN}/config/ember_agent_copy_canary_v0.0.13.json"
CONFIG_SHA256 = "1e1c164cc4d8e46550fe5cf16babe3944f81f4cc6dacb7a4b55ac65a283bb0ed"
DATA_URL = f"https://raw.githubusercontent.com/jmiller18899-lab/Ember-llm/{ASSET_PIN}/jobs/ember_sft_data_v013.py"
DATA_SHA256 = "cbaa8dfeaab5cb42fb38c00b0118972982e1979135b919115daf926a375a80ad"
...
def load_module(path: Path):
    spec = importlib.util.spec_from_file_location("ember_sft_data_v013", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("unable to load the v0.0.13 data module")
...
def assert_no_live_duplicate(state: dict | None) -> None:
    ...
    if status in {"training_complete_pending_eval", "evaluation_complete", "complete"}:
        raise RuntimeError("v0.0.13 canary is already complete; refusing a duplicate paid run")
    if status != "running":
        return
    updated_at = str(state.get("updated_at") or state.get("started_at") or "")
    if not updated_at:
        raise RuntimeError("existing v0.0.13 run lock has no timestamp; manual review required")
    age = (utc_now() - parse_timestamp(updated_at)).total_seconds()
    if age < RUN_STATE_STALE_SECONDS:
        raise RuntimeError(f"another v0.0.13 canary appears active (state age {age:.0f}s)")


def main() -> None:
    parser = argparse.ArgumentParser(description="Run or preflight Ember v0.0.13 short-copy canary")
    ...
    if ASSET_PIN == "CURSOR_PIN_PLACEHOLDER":
        raise RuntimeError("v0.0.13 asset pin is unset; refuse to launch an unpinned job")

    import torch

    api = HfApi(token=token)
    with tempfile.TemporaryDirectory(prefix="ember-v013-") as temporary:
        work = Path(temporary)
        config_path = fetch(CONFIG_URL, work / "config.json", CONFIG_SHA256)
        data_path = fetch(DATA_URL, work / "data.py", DATA_SHA256)
        config = json.loads(config_path.read_text(encoding="utf-8"))
        if config.get("version") != "0.0.13" or config.get("phase") != "short-copy-canary":
            raise RuntimeError("unexpected Ember v0.0.13 configuration")
        if config.get("source_model_name") != "ember-v0.0.9-t4":
            raise RuntimeError("v0.0.13 must initialize from the accepted v0.0.9 checkpoint")
        if float(config["semantic_token_weight"]) > 4.0:
            raise RuntimeError("v0.0.13 refuses the 8x semantic weight that collapsed v0.0.12")
        if int(config["max_steps"]) < 80:
            raise RuntimeError("v0.0.13 refuses another 40-step canary")
        ...
        if args.preflight_only:
            print("EMBER_HF_V013_SFT_PREFLIGHT=PASS", flush=True)
            return

        if not torch.cuda.is_available():
            raise RuntimeError("Ember v0.0.13 canary requires the explicitly approved T4 GPU")
        ...
        upload(api, repo, write_json(work / "run-state.json", state), "run-state.json", "Start Ember v0.0.13 short-copy canary")
        ...
        try:
            for step in range(max_steps):
                ...
                if (step + 1) % int(config["hub_checkpoint_interval"]) == 0:
                    upload(api, repo, latest_path, "resume/latest.pt", f"Save v0.0.13 resume step {step}")
            ...
            upload(api, repo, write_json(work / "run-state.json", state), "run-state.json", "Record interrupted Ember v0.0.13 canary")
            raise
        ...
        if not Path(int4_path).is_file() or Path(int4_path).stat().st_size == 0:
            raise RuntimeError("v0.0.13 INT4 export failed")
        ...
        frozen_config = work / "ember_agent_copy_canary_v0.0.13.json"
        frozen_config.write_text(config_path.read_text(encoding="utf-8"), encoding="utf-8")
        upload(api, repo, best_path, f"checkpoints/{run_id}/best.pt", "Upload v0.0.13 best checkpoint")
        upload(api, repo, Path(int4_path), f"checkpoints/{run_id}/best.int4.pt", "Upload v0.0.13 INT4 checkpoint")
        upload(api, repo, latest_path, f"checkpoints/{run_id}/latest.pt", "Upload v0.0.13 latest checkpoint")
        upload(api, repo, frozen_config, "config/ember_agent_copy_canary_v0.0.13.json", "Record v0.0.13 config")
        upload(api, repo, summary_path, "training-summary.json", "Record v0.0.13 training summary")
        ...
        upload(api, repo, write_json(work / "run-state.json", state), "run-state.json", "Update Ember v0.0.13 run state")
        print(json.dumps(summary, indent=2, sort_keys=True), flush=True)
        print(f"MODEL_REPO={repo}", flush=True)
        print(f"EMBER_HF_V013_SFT={'PASS' if smoke['passed'] else 'FAIL'}", flush=True)
        if not smoke["passed"]:
            raise RuntimeError("v0.0.13 internal copy smoke failed; do not run promotion evaluation")
