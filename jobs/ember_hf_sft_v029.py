...
# ///
"""Ember v0.0.29: boundary focus, from the v0.0.27 checkpoint.
...
# The v0.0.26 curriculum, carried unchanged through v0.0.27 and v0.0.29.
DATA_COMMIT = "36a213d9fecaf9ab8348132d6036e9c99a17e18d"
# The commit carrying config/ember_multi_position_v0.0.29.json.
CONFIG_COMMIT = "c9dc3832de15ee3db247bce006c8fe3650266430"
BASE_URL = (
...
HELPERS = r'''
def _v029_field_signature(value):
...
def assert_format_parity(data, train_rows):
    ...
    if missing:
        raise RuntimeError(
            "v0.0.29 format parity failed; the curriculum cannot emit these held-out "
            "templates: %s" % missing
        )
    return {
...
def v029_supervised_span(y, weights, cfg, torch):
...
def v029_gaps(logits, y, mask, torch):
...
def v029_row_worst(gap, mask, cfg, torch):
...
def v029_boundary_weight(worst_gap, cfg, torch):
...
def multi_position_loss(model, x, y, weights, cfg, torch):
    ...
    copy_mask, span_mask = v029_supervised_span(y, weights, cfg, torch)
    if not bool(copy_mask.any().item()):
        return ce

    copy_gap, copy_best_is_correct = v029_gaps(logits, y, copy_mask, torch)
    ...
    span_gap, _ = v029_gaps(logits, y, span_mask, torch)
    worst_k, worst_gap, has_targets = v029_row_worst(span_gap, span_mask, cfg, torch)
    focus, _ = v029_boundary_weight(worst_gap, cfg, torch)
...
def sample_multi_position_indices(train, batch_size, cfg, generator, model, device, torch):
    ...
    with torch.no_grad():
        logits, _ = model(x, None)
        copy_mask, span_mask = v029_supervised_span(y, w, cfg, torch)
...
def v029_sequence_stats(model, tokenizer, prompt, expected, device, torch, base):
...
def _v029_percentiles(values, points):
    if not values:
        return {}
...
def _v029_histogram(values, edges):
    counts = {}
...
def v029_battery(model, tokenizer, cases, prompt_fn, cfg, device, torch, base):
    model.eval()
    rows = []
    correct_tokens = 0
    total_tokens = 0
    for kind, value, corrupt in cases:
        ...
        stats = v029_sequence_stats(model, tokenizer, prompt, value, device, torch, base)
    ...
    distribution = {
        "span_min_gap_percentiles": _v029_percentiles(
            span_gaps, cfg["margin_report_percentiles"]
        ),
        "span_min_gap_histogram": _v029_histogram(span_gaps, cfg["margin_histogram_edges"]),
    ...
    for row in rows:
        for label, table in (
            (row["kind"], by_kind),
            (_v029_bucket(row["continuation_tokens"], edges), by_length),
        ):
...
def v029_meets(value, threshold, cfg):
...
def v029_legacy_continuation_tokens(model, tokenizer, cfg, device, torch, base):
...
def v029_band_flow(baseline, final, cfg):
...
def v029_gate_distance(final, cfg):
    ...
    for key, denominator, gate, unit in checks:
        ...
        need = have
        while need <= denominator and not v029_meets(need / denominator, gate, cfg):
            need += 1
        out[key] = {
...
def v029_diagnostic(model, tokenizer, data, cfg, device, torch, base):
    legacy = diagnostic(model, tokenizer, cfg, device, torch, base)
    legacy_correct, legacy_total = v029_legacy_continuation_tokens(
        model, tokenizer, cfg, device, torch, base
    )
    expanded = v029_battery(
...
def v029_progress(baseline, final, cfg):
...
TRANSFORMS = (
    ...
    ("0.0.16", "0.0.29", -1),
    ("ember_sequence_copy_v0.0.29.json", "ember_multi_position_v0.0.29.json", -1),
    ("ember_sft_data_v015.py", "ember_sft_data_v026.py", -1),
    ("sequence-copy-consolidation", "multi-position-repair", -1),
    ('SOURCE_REPO = "Jmiller18899/ember-v0.0.15-t4"',
     'SOURCE_REPO = "Jmiller18899/ember-v0.0.28-t4"', -1),
    ('cfg.get("source_model_name") != "ember-v0.0.15-t4"',
     'cfg.get("source_model_name") != "ember-v0.0.28-t4"', -1),
    ("must start from v0.0.15", "must start from v0.0.28", -1),
    ('if str(source_cfg.get("version")) != "0.0.15":',
     'if str(source_cfg.get("version")) != "0.0.28":', -1),
    ("expected a v0.0.15 checkpoint", "expected a v0.0.28 checkpoint", -1),
    ("v0.0.15 checkpoint run_id", "v0.0.28 checkpoint run_id", -1),
    ("v0.0.15 source state", "v0.0.28 source state", -1),
    ("resolve_v015_source", "resolve_v028_source", -1),
    ("v015_promotion", "v028_promotion", -1),
    ("v015_best_step", "v028_best_step", -1),
    ...
    ('baseline = diagnostic(model, tokenizer, cfg, "cpu", torch, base)',
     'baseline = v029_diagnostic(model, tokenizer, data, cfg, "cpu", torch, base)', 1),
    ("diag = diagnostic(model, tokenizer, cfg, device, torch, base)",
     "diag = v029_diagnostic(model, tokenizer, data, cfg, device, torch, base)", 1),
    ("final = diagnostic(model, tokenizer, cfg, device, torch, base)",
     "final = v029_diagnostic(model, tokenizer, data, cfg, device, torch, base)", 1),
    ("current = score(diag, val_loss)", "current = v029_score(diag, val_loss)", 1),
    ...
    ('if args.preflight_only:\n'
     ...
     '            if len(probe) != 2:\n'
     '                raise RuntimeError("v0.0.29 boundary mining preflight returned wrong batch size")\n'
     '            reach = baseline["metrics"]["expanded_sequences_within_reach"]\n'
     '            print("EMBER_V029_FORMAT_PARITY=PASS", flush=True)\n'
     '            print("EMBER_V029_BOUNDARY_MINING=PASS", flush=True)\n'
     '            print(f"EMBER_V029_SEQUENCES_WITHIN_REACH={reach:.4f}", flush=True)\n'
     '            print(json.dumps(baseline["distribution"], indent=2, sort_keys=True), flush=True)\n'
     '            print("EMBER_V029_GATE_DISTANCE", flush=True)\n'
     '            print(json.dumps(v029_gate_distance(baseline, cfg), indent=2, sort_keys=True), flush=True)\n'
     '            if reach <= 0.0:\n'
     '                print("EMBER_V029_ADVICE=NO_REACHABLE_SEQUENCES_DO_NOT_LAUNCH_T4", flush=True)\n'
     '            print("EMBER_HF_V029_PREFLIGHT=PASS", flush=True)\n'
     '            return', 1),

    ("best_score = None\n        best_step = -1\n        history = []",
     "best_score = v029_score(baseline, -1e9)\n"
     "        best_step = -1\n"
     "        history = []\n"
    ...
    ('"promotion": "PASS" if final["passed"] else "FAIL",',
     '"format_parity": parity,\n'
     '            "progress": v029_progress(baseline, final, cfg),\n'
     '            "baseline_distribution": baseline["distribution"],\n'
    ...
    ('print(f"EMBER_HF_V016_PROMOTION={report[\'promotion\']}", flush=True)',
     'print(f"EMBER_HF_V029_PROMOTION={report[\'promotion\']}", flush=True)\n'
     '        print(f"EMBER_V029_PROGRESS={report[\'progress\'][\'verdict\']}", flush=True)\n'
     '        print(json.dumps(report["progress"]["deltas"], indent=2, sort_keys=True), flush=True)\n'
     '        print("EMBER_V029_GATE_DISTANCE", flush=True)\n'
     '        print(json.dumps(report["progress"]["gate_distance"], indent=2, sort_keys=True), flush=True)\n'
     '        print("EMBER_V029_BAND_FLOW", flush=True)\n'
     '        print(json.dumps(report["progress"]["band_flow"], indent=2, sort_keys=True), flush=True)\n'
     '        print(json.dumps(report["final_distribution"], indent=2, sort_keys=True), flush=True)', 1),

    ("EMBER_HF_V016_", "EMBER_HF_V029_", -1),
    ("EMBER_V016_", "EMBER_V029_", -1),
)
...
def apply_transforms(source: str) -> str:
    for old, new, count in TRANSFORMS:
        if old not in source and old not in OPTIONAL_TARGETS:
            raise RuntimeError(f"v0.0.29 transform target missing: {old!r}")
...
def main() -> None:
    ...
    print("EMBER_V029_BOUNDARY_FOCUS_TRANSFORM=PASS", flush=True)
    exec(compile(source, "ember_hf_sft_v029_wrapper_runtime.py", "exec"), {"__name__": "__main__"})
