"""Compare definition routing with the frozen v8 control; never retune while scoring."""
import argparse
...
from .evaluate_v8 import parser_regressions
from .evidence_v5 import emit_json
from .freeze_v9 import verify_confirmation, verify_freeze
from .runtime import sha256
from .runtime_v8 import RoutingV8Runtime
from .runtime_v9 import RoutingV9Runtime

DEVELOPMENT_SUITES = ("confirmation-100", "routing-v5-confirmation", "routing-v6-confirmation", "routing-v7-confirmation", "routing-v8-confirmation")
...
def main():
    ...
    name = args.suite or "routing-v9-confirmation"
    ...
    for precision in ("full", "int4"):
        results[precision] = {}
        for arm, cls in (("frozen_v8", RoutingV8Runtime), ("contact_v9", RoutingV9Runtime)):
            runtime = cls(args.bundle, precision)
            result = evaluate(runtime, suite["cases"])
            results[precision][arm] = result
            del runtime
            print(json.dumps({"event": "routing_v9_measurement", "precision": precision, "arm": arm,
    ...
    passed = all(r["contact_v9"]["combined_correct"] == 100 for r in results.values())
    ...
    print(json.dumps({"event": "routing_v9_summary", "suite": name, "kind": args.kind,
