"""Separate paired routing and parser gains; never fit or retune in scoring."""
import argparse
...
from .freeze_v7 import verify_confirmation, verify_freeze
from .runtime import sha256
from .runtime_v6 import RoutingV6Runtime
from .runtime_v7 import ParserV7Control, RoutingV7Runtime

DEVELOPMENT_SUITES = ("confirmation-100", "routing-v5-confirmation", "routing-v6-confirmation")
...
def main():
    ...
    name = args.suite or "routing-v7-confirmation"
    ...
    for precision in ("full", "int4"):
        results[precision] = {}
        for arm, cls in (("frozen_v6", RoutingV6Runtime), ("v6_with_parser_v7", ParserV7Control),
                         ("request_v7", RoutingV7Runtime)):
            runtime = cls(args.bundle, precision)
            result = evaluate(runtime, suite["cases"])
            results[precision][arm] = result
            del runtime
            print(json.dumps({"event": "routing_v7_measurement", "precision": precision, "arm": arm,
    ...
    passed = all(r["request_v7"]["combined_correct"] == 100 for r in results.values())
    ...
    print(json.dumps({"event": "routing_v7_summary", "suite": name, "kind": args.kind,
